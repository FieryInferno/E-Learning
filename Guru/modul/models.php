<?php 
  ...
  if (isset($_POST['mapelSave'])) {
    ...
    $kelas    = $_POST['kelas'];
    $mapel    = $_POST['mapel'];
    $sql      = mysqli_query($con, "INSERT INTO tb_roleguru VALUES (NULL, '$id_guru', '$kelas', '$mapel')") or die(mysqli_error($con));
    if ($sql) {
      echo "
    ...
    $sql  = mysqli_query($con,"UPDATE tb_roleguru SET id_kelas='$_POST[kelas]',id_mapel='$_POST[mapel]' WHERE id_roleguru='$_POST[ID]' ") or die(mysqli_error($con));
    if ($sql) {
      echo "
        <script type='text/javascript'>
          setTimeout(function () {
    ...
    $sql  = mysqli_query($con,"UPDATE ujian SET judul='$_POST[judul]',tanggal='$_POST[tgl]',waktu='$_POST[waktu]',jml_soal='$_POST[jumlah]',acak='$_POST[acak]',tipe='0',id_jenis='$_POST[id_jenis]',id_mapel='$_POST[id_mapel]' WHERE id_ujian='$_POST[id]' ");
    if ($sql) {
      echo "
        <script type='text/javascript'>
          setTimeout(function () {
    ...
    for ($x=0; $x <$jumlahTerpilih ; $x++) { 
      $s  = mysqli_query($con, "INSERT INTO kelas_ujian(id_ujian,id_kelas,id_jurusan,aktif) VALUES('$_POST[id]','$kls[$x]','$jrs[$x]','Y')") or die(mysqli_error($con));
      if ($s) {
        echo "
          <script>
    ...
    $jumlahTerpilih	= count($kls);
    for ($x=0; $x <$jumlahTerpilih ; $x++) { 
      $s  = mysqli_query($con, "INSERT INTO kelas_ujianessay(id_ujianessay,id_kelas,id_jurusan,aktif) VALUES('$_POST[id]','$kls[$x]','$jrs[$x]','Y')") or die(mysqli_error($con));
      if ($s) {
        echo "
          <script>
            window.location='?page=ujian';
    ...
    $sql= mysqli_query($con, "INSERT INTO soal(id_ujian, soal, pilihan_1, pilihan_2, pilihan_3, pilihan_4, pilihan_5, kunci) VALUES('$_POST[id]', '$_POST[soal]', '$_POST[p1]', '$_POST[p2]', '$_POST[p3]', '$_POST[p4]', '$_POST[p5]', '$_POST[kunci]')");
    if ($sql) {
      echo "
        <script type='text/javascript'>
          setTimeout(function () {
    ...
    $sql  = mysqli_query($con,"INSERT INTO tb_tugas VALUES(NULL, '$_POST[id_jenis]', '$_POST[judul]', '$_POST[isi_tugas]', '$_POST[tgl]', '$_POST[waktu]', '$_POST[jumlahanggota]', '$_POST[id_guru]', '$_POST[id_mapel]', NULL ) ") or die(mysqli_error($con));
    if ($sql) {
      echo "
        <script type='text/javascript'>
          setTimeout(function () {
